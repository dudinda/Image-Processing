...
***
Introducing a page controller to navigate inside one window and  a front controller with a http-context to accept a web request, it may be possible to unify mvp, mvc and mvvm under the single abstraction named engine with the 3 singletons. The entry point may contain a state machine to control application lifecycle and a cross platfrom server. Using .NET and COM interoperability there is no need for cross-platform xaml forms.                   
***                      
